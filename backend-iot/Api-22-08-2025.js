...
// Esquema de Temperatura (temperatura no requerido)
const EsquemaTemperatura = new mongoose.Schema({
  idAuto: { type: Number, unique: true },
  fechaHora: { type: Date, default: Date.now },
  temperatura: { type: Number }, // No requerido
...
// Endpoint de temperatura (POST) - temperatura no requerido
...
// Nuevo endpoint: GET por deviceId (para detalles y gráficos)
...
app.get("/fatiga", autenticar, async (req, res) => {
  try {
    ...
    const datosConUrl = datos.map((dato) => {
      const imageUrl = dato.foto
        ? `https://${req.get("host")}${dato.foto}`
        : null;
      console.log(`Imagen para fatiga ${dato._id}: ${imageUrl}`);
      return {
        ...dato._doc,
        foto: imageUrl,
      };
...
app.delete("/fatiga/:id", autenticar, async (req, res) => {
  ...
  try {
    ...
    if (dato.foto) {
      try {
        const filePath = path.join(__dirname, dato.foto);
...
// Endpoint opcional por idDispositivo (GET) - legacy, usa el nuevo /device/
app.get(
