...
app.post("/temperatura", autenticar, async (req, res) => {
  const { ambiente, idDispositivo, temperatura, ventilador } = req.body;
  if (!ambiente || !idDispositivo || temperatura === undefined) {
    return res.status(400).json({
      mensaje: "Ambiente, ID de dispositivo y temperatura son requeridos",
    });
  }
  try {
    const dato = new Temperatura({
      ...
      conectadoVentilador:
        ventilador !== undefined ? ventilador : temperatura > 30,
    });
    await dato.save();
